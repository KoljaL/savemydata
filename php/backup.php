...
$backup_file_name = $backup_folder.'/'.date( "Y_m_d_H_i" ).'.zip';
...
// Backup function
/**
 * This function creates a zip archive of a folder and all its contents
 *
 * @param folder_to_backup The folder you want to backup.
 * @param backup_file_name The name of the backup file.
 * @param not_to_backup an array of files and folders that you don't want to backup.
 *
...
if ( $files ) {
    // check for backup today
    foreach ( $files as $file ) {
        if ( date( "Y_m_d_H_i" ).'.zip' === $file ) {
