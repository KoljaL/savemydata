...
{
    global $db, $API_param, $API_value, $user_id, $user_role;
    $sharingEmail_ID = false;
    //TODO shareID.staff_id !== $user_id -> not allowed to share
    ...
    if ($sharingEmail_ID) {
        ...
        // for response
        $data['itemName']  = get_by_from($itemName, 'id', $param['shared_id'], $API_param);
        $data['staffName'] = get_by_from('username', 'id', $sharingEmail_ID, 'staff');

        // saves the sharing in DB and get back a value if its a new or a updated entry
        $data['state'] = sharing($API_param.'_sharing', $param['shared_id'], $sharingEmail_ID, $user_id, $param['can_edit']);
    }
